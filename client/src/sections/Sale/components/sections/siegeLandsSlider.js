...
function SiegeLandsSlider({ info, val, cards, counter }) {
        ...
        <div className='info-wrapper slider-info'>
          {val !== 'PACKS' ?
            <>
              ...
              <div className='info-line'>
                <p className='info-line-label'>TOTAL DEAD ORCS</p>
            ...
            <>
              <div className='info-line'>
                <p className='info-line-label'>ENDLESS SIEGE LANDS</p>
              </div>
              <div className='info-line'>
  ...
  return (
    <div className="buy-citizen-card card-box-new">
      <p className='title'>{val !== "PACKS" ? `${val} LANDS` : val}</p>
      {renderSlides(Object.values(info).flat().filter(value => value.rarity === val))}
    </div>
  )
}

export default SiegeLandsSlider